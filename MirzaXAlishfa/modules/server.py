...
async def handle_play_command(_, message: Message):
    ...
    if room['current']:
        ...
        await message.reply_text(chat_id, f"▶️ Now playing: {song['title']}")
...
async def skip_track(_, message: Message):
    ...
    if room['current']:
        ...
        message.reply_text(chat_id, f"▶️ Now playing: {song['title']}")
    else:
...
def handle_user_joined(user):
    ...
    current = ROOMS.get(chat_id, {}).get("current")
    if current:
        start_time = current.get("start_time")
        if start_time:
            elapsed = time.time() - start_time
            emit("resume_song", {
                "stream_url": current["path"],
                "title": current["title"],
                "author": current["author"],
                "offset": round(elapsed, 2)
            })

    room_users[chat_id][user_id] = user

    print(f"[+] {user['first_name']} joined VC in chat {chat_id}")

    emit('participant_list', list(room_users[chat_id].values()), to=request.sid)
...
def handle_disconnect():
    ...
    if sid in socket_user_map:
        ...
        leave_room(chat_id)
        if chat_id in room_users:
            user = room_users[chat_id].pop(user_id, None)
            if user:
                emit('participant_left', user, room=chat_id)
